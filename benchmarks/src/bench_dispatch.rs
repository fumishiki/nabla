...
use nabla::prelude::*;
use nabla_bench::{gpu_sync, rand_tensor};
...
fn main() {
    ...
    for &(op, count) in &[("exp", 1000usize), ("add", 1000)] {
        ...
        println!(
            "{{\"test\":\"burst\",\"op\":\"{op}\",\"count\":{count},\"ns_per_op\":{ns_per_op:.0},\"total_ms\":{total_ms:.3}}}"
        );
        drop(ring);
    }
    ...
    println!(
        "{{\"test\":\"fuse_vs_unfuse\",\"unfused_ms\":{unfused_ms:.3},\"fused_ms\":{fused_ms:.3},\"speedup\":{speedup:.2},\"iters\":{iters}}}"
    );
    ...
    println!(
        "{{\"test\":\"training_step\",\"mode\":\"eager\",\"steps\":{steps},\"total_ms\":{eager_ms:.3},\"us_per_step\":{eager_us_per_step:.1}}}"
    );
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        if capture_ok {
            ...
            if replay_ok {
                ...
                println!(
                    "{{\"test\":\"training_step\",\"mode\":\"cuda_graph\",\"steps\":{steps},\"total_ms\":{graph_ms:.3},\"us_per_step\":{graph_us_per_step:.1},\"speedup\":{graph_speedup:.2}}}"
                );
            } else {
                println!(
                    "{{\"test\":\"training_step\",\"mode\":\"cuda_graph\",\"status\":\"replay_failed\"}}"
                );
            }
        } else {
            println!(
                "{{\"test\":\"training_step\",\"mode\":\"cuda_graph\",\"status\":\"capture_failed\"}}"
            );
        }
    }
    ...
    eprintln!(
        "Fuse 4-op speedup: {speedup:.2}x ({unfused_ms:.3} ms -> {fused_ms:.3} ms over {iters} iters)"
    );
