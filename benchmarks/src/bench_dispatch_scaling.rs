...
use nabla::prelude::*;
use nabla_bench::{bench_ms, gpu_sync, kaiming, rand_tensor};
...
fn exp1_op_chain() {
    ...
    for &sz in &SIZES {
        ...
        println!(
            "{{\"test\":\"exp1_chain\",\"size\":{sz},\"ops\":{CHAIN_OPS},\"total_ms\":{total_ms:.3},\"ops_per_sec\":{ops_per_sec:.0},\"ns_per_op\":{ns_per_op:.0}}}"
        );
...
fn exp2_mlp_forward() {
    ...
    for &(batch, iters) in &[(1usize, 500usize), (32, 100), (128, 50), (1024, 20)] {
        ...
        println!(
            "{{\"test\":\"exp2_forward\",\"batch\":{batch},\"ms_per_fwd\":{ms_per_fwd:.4},\"fwd_per_sec\":{fwd_per_sec:.0},\"iters\":{iters}}}"
        );
...
fn exp3_train_step() {
    ...
    for &(batch, steps) in &[(1usize, 100usize), (32, 30), (128, 10)] {
        ...
        let mut do_step = |w1: &mut Tensor<f32>, w2: &mut Tensor<f32>, w3: &mut Tensor<f32>| {
            let tape = Tape::new();
            if let (Ok(x_var), Ok(t_var)) =
                (tape.variable(x.clone()), tape.variable(target.clone()))
            {
        ...
        println!(
            "{{\"test\":\"exp3_train\",\"batch\":{batch},\"ms_per_step\":{ms:.4},\"steps_per_sec\":{steps_per_sec:.0},\"steps\":{steps}}}"
        );
