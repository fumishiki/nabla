...
use nabla::prelude::*;
use nabla_bench::{gpu_sync, kaiming, must, rand_tensor};
...
fn main() {
    ...
    for &batch in &[1usize, 32, 128] {
        ...
        let (mut t_tape, mut t_fwd, mut t_bwd, mut t_grad, mut t_sgd) =
        ...
        for _ in 0..steps {
            ...
            gpu_sync();
            let t2 = Instant::now();
            t_fwd += (t2 - t1).as_nanos();

            let _ = loss.backward_unchecked();
            gpu_sync();
            let t3 = Instant::now();
            t_bwd += (t3 - t2).as_nanos();
            ...
            gpu_sync();
            let t4 = Instant::now();
            t_grad += (t4 - t3).as_nanos();

            sgd.step(&mut [&mut w1, &mut w2, &mut w3], &[&*g1, &*g2, &*g3]);
            gpu_sync();
            let t5 = Instant::now();
            t_sgd += (t5 - t4).as_nanos();
        }
        ...
        println!(
            "  TOTAL:      {:.4} ms",
            to_ms(t_tape + t_fwd + t_bwd + t_grad + t_sgd)
        );
        println!();
    }
}
