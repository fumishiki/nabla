...
use nabla_bench::{bench_ms, kaiming, must, rand_tensor};
...
fn main() {
    println!(
        "nabla MLP benchmark  784->256->128->10, leaky_relu(0.01), MSE sum, SGD lr=0.001, f32"
    );
    ...
    for &batch in BATCH_SIZES {
        ...
        let eager_ms = bench_ms(WARMUP, ITERS, || {
            train_step(&x, &target, &mut w1, &mut w2, &mut w3, &mut sgd)
        });
        ...
        let graph_ms = {
            ...
            match cuda_graph_capture(|| {
                ...
                Err(e) => {
                    eprintln!("  batch={batch} capture failed: {e}");
                    f64::NAN
                }
            }
        };
        ...
        if graph_ms.is_finite() {
            println!(
                "  cuda_graph: {graph_ms:.4} ms/step  ({:.2}x)",
                eager_ms / graph_ms
            );
        } else {
    ...
    for &(batch, eager, graph) in &results {
        let g = if graph.is_finite() {
            format!("{graph:.4}")
        } else {
            "N/A".to_string()
        };
        let s = if graph.is_finite() {
            format!("{:.2}x", eager / graph)
        } else {
            "-".to_string()
        };
