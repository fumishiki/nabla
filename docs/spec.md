...
| `fuse!` | ✅ CPU chain | ✅ GPU kernel | tensor method chain |
...
|---|---|---|---|
| Binary | add, sub, mul_elem, div_elem, scale | `elementwise_binary` | `elementwise_binary` |
| Unary | exp, ln, log1p, sin, cos, tanh, sqrt, abs, recip, erf, ceil, floor, round, powf, neg | `elementwise_unary` | `elementwise_unary` |
...
| Construction | zeros, fill, identity | `fill`, `identity` | `fill`, `identity` |
...
```rust
// CUDA path (gpu_cuda.rs)
fn compile_kernel(source: &str) -> CUmodule {
    let nvrtc = libloading::Library::new("libnvrtc.so")?;
    let program = nvrtcCreateProgram(source, ...);
    nvrtcCompileProgram(program, &["--gpu-architecture=compute_80"]);
    let ptx = nvrtcGetPTX(program);
    cuModuleLoadData(&ptx)  // → CUmodule
}
```

- No CUDA SDK at compile time — `libloading` dynamically loads `libcuda.so` + `libnvrtc.so`
- Compiled PTX cached in `HashMap<&'static str, CUmodule>` (same pattern as wgpu pipeline cache)
- HIP path identical: `libamdhip64.so` + `libhiprtc.so`
...
**Kernel fusion** — `fuse!` proc macro already has the full op chain at compile time. Instead of emitting chained dispatch calls, emit a single fused shader (WGSL or CUDA/HIP C) where intermediates stay in SRAM. Implementation levels:

| Level | Scope | Paper |
|---|---|---|
| L1: Element-wise fusion | Fuse consecutive unary/binary ops | Liger [2410.10989] |
| L2: Reduction fusion | Fuse across reduction ops with loop-carried deps | Neptune [2510.08726] |
| L3: GEMM+pointwise fusion | Fuse matmul + activation + normalization | Deep Kernel Fusion [2602.11808] |
...
All spec items implemented. No remaining items.
