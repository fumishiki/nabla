...
fn wht_impl<T: Scalar, B: BackendCore + ?Sized>(a: &B::Storage<T>, inverse: bool) -> B::Storage<T> {
    ...
    for r in 0..rows {
        ...
        if inverse {
            for v in row.iter_mut() { *v = *v * inv_n; }
        }
    }
...
pub trait BackendNN: BackendCore {
    ...
    fn batch_norm_train<T: Scalar>(
        ...
        if training {
            ...
            for c in 0..cols {
                Self::set(running_mean, 0, c, one_minus * Self::get(running_mean, 0, c) + momentum * mean[c]);
                Self::set(running_var, 0, c, one_minus * Self::get(running_var, 0, c) + momentum * var[c]);
            }
        }
        Self::from_fn(rows, cols, |r, c| {
            (Self::get(a, r, c) - mean[c]) / (var[c] + eps).math_sqrt()
                * Self::get(gamma, 0, c) + Self::get(beta, 0, c)
        })
    }
    ...
    fn cross_entropy_fused<T: Scalar>(
        ...
        let total = (0..n).fold(T::zero(), |acc, i| {
            ...
            let sum_exp = (0..c).fold(T::zero(), |s, j| s + (Self::get(input, i, j) - row_max).math_exp());
    ...
    fn embedding<T: Scalar>(indices: &Self::Storage<T>, weight: &Self::Storage<T>) -> Self::Storage<T>;

    /// Embedding backward: scatter-add grad into weight rows.
    fn embedding_backward<T: Scalar>(
        indices: &Self::Storage<T>, grad: &Self::Storage<T>, vocab: usize,
    ) -> Self::Storage<T> {
    ...
    fn max_pool2d<T: Scalar>(
        a: &Self::Storage<T>, h: usize, w: usize, kh: usize, kw: usize,
    ...
    fn max_pool2d_with_indices<T: Scalar>(
        ...
        for n in 0..nc {
            for oh in 0..out_h {
                for ow in 0..out_w {
                    let (mut best, mut best_flat, mut found) = (T::from_f64(f64::NEG_INFINITY), 0usize, false);
                    for khr in 0..kh {
                        for kwc in 0..kw {
                            let (ih, iw) = (oh * sh + khr, ow * sw + kwc);
                            if ih >= ph && ih < h + ph && iw >= pw && iw < w + pw {
                                ...
                                if !found || crate::scalar::ReductionOps::reduction_gt(v, best) {
                                    best = v; best_flat = fi; found = true;
                                }
                            }
                        }
                    }
    ...
    fn adaptive_avg_pool2d<T: Scalar>(
        ...
        Self::from_fn(nc, out_h * out_w, |n, op| {
            ...
            let (mut sum, mut cnt) = (T::zero(), 0usize);
            for ih in ih_s..ih_e { for iw in iw_s..iw_e { sum = sum + Self::get(a, n, ih * in_w + iw); cnt += 1; } }
            if cnt == 0 { T::zero() } else { sum / T::from_f64(cnt as f64) }
        })
    }
    ...
    fn conv2d<T: Scalar>(
        ...
        Self::from_fn(n * c_out, out_h * out_w, |row, col| {
            ...
            let mut acc = T::zero();
            for ic in 0..c_in_g { for khr in 0..kh { for kwc in 0..kw {
                let (ih, iw) = (oh * stride.0 + khr * dilation.0, ow * stride.1 + kwc * dilation.1);
                if ih >= padding.0 && ih < h + padding.0 && iw >= padding.1 && iw < w + padding.1 {
                    acc = acc + Self::get(input, b * c_in + g * c_in_g + ic, (ih - padding.0) * w + (iw - padding.1))
    ...
    fn conv1d<T: Scalar>(
        ...
        Self::from_fn(n_batch * c_out, out_len, |row, col| {
            ...
            let mut acc = T::zero();
            for ic in 0..c_in_g { for k in 0..kernel_size {
                let il = col * stride + k * dilation;
                if il >= padding && il < length + padding {
                    acc = acc + Self::get(input, b * c_in + g * c_in_g + ic, il - padding)
    ...
    fn conv3d<T: Scalar>(
        ...
        Self::from_fn(n_batch * c_out, out_d * out_h * out_w, |row, col| {
            ...
            let mut acc = T::zero();
            for ic in 0..c_in_g { for kdr in 0..kd { for khr in 0..kh { for kwc in 0..kw {
                let (id, ih, iw) = (od * stride.0 + kdr * dilation.0, oh * stride.1 + khr * dilation.1, ow * stride.2 + kwc * dilation.2);
                if id >= padding.0 && id < d + padding.0 && ih >= padding.1 && ih < h + padding.1 && iw >= padding.2 && iw < w + padding.2 {
                    acc = acc + Self::get(input, b * c_in + g * c_in_g + ic,
                        (id - padding.0) * h * w + (ih - padding.1) * w + (iw - padding.2))
                        * Self::get(weight, oc, ic * kd * kh * kw + kdr * kh * kw + khr * kw + kwc);
                }
            }}}}
            acc
        })
    ...
    fn conv_transpose2d<T: Scalar>(
        input: &Self::Storage<T>, weight: &Self::Storage<T>,
        n_batch: usize, c_in: usize, h: usize, w: usize, c_out: usize,
        kh: usize, kw: usize, stride: (usize, usize), padding: (usize, usize),
        ...
        Self::from_fn(n_batch * c_out, out_h * out_w, |row, col| {
            ...
            let (oh, ow) = (col / out_w, col % out_w);
            let mut acc = T::zero();
            for ic in 0..c_in { for khr in 0..kh { for kwc in 0..kw {
                    ...
                    && (iw_pad - kwc).is_multiple_of(stride.1)
                {
                    ...
                    if ih < h && iw < w {
                        acc = acc + Self::get(input, b * c_in + ic, ih * w + iw)
    ...
    fn relu_backward<T: Scalar>(grad: &Self::Storage<T>, input: &Self::Storage<T>) -> Self::Storage<T> {
        ...
        Self::from_fn(m, n, |r, c| if Self::get(input, r, c).to_f64() > 0.0 { Self::get(grad, r, c) } else { T::zero() })
    }
    ...
    fn gelu_backward<T: Scalar>(grad: &Self::Storage<T>, input: &Self::Storage<T>) -> Self::Storage<T> {
        let (m, n) = (Self::nrows(grad), Self::ncols(grad));
        let (inv_sqrt2, inv_sqrt_2pi) = (T::from_f64(std::f64::consts::FRAC_1_SQRT_2), T::from_f64(1.0 / (2.0 * std::f64::consts::PI).sqrt()));
    ...
    fn wht<T: Scalar>(a: &Self::Storage<T>) -> Self::Storage<T> { wht_impl::<T, Self>(a, false) }
    /// Inverse Walsh-Hadamard Transform: butterfly + normalize by 1/cols.
    fn wht_inverse<T: Scalar>(a: &Self::Storage<T>) -> Self::Storage<T> { wht_impl::<T, Self>(a, true) }
    ...
    fn sdpa<T: Scalar>(
        ...
        _mask: Option<&Self::Storage<T>>, seq_q: usize, seq_k: usize,
        head_dim: usize, batch_heads: usize,
    ) -> Self::Storage<T> {
        let scale = T::from_f64(1.0 / (head_dim as f64).sqrt());
        let mut out = Self::from_fn(batch_heads * seq_q, head_dim, |_, _| T::zero());
        for bh in 0..batch_heads {
            for i in 0..seq_q {
                let scores: Vec<T> = (0..seq_k).map(|j| {
