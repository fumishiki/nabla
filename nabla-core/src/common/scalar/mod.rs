...
/// ReductionOps for types that compare by a `.value` field (Dual, MultiDual).
macro_rules! impl_value_reduction {
    ([$($gen:tt)*] $ty:ty) => {
        impl<$($gen)*> ReductionOps for $ty {
            #[inline] fn reduction_add(self, other: Self) -> Self { self + other }
            #[inline] fn reduction_max(self, other: Self) -> Self { if self.value > other.value { self } else { other } }
            #[inline] fn reduction_min(self, other: Self) -> Self { if self.value < other.value { self } else { other } }
            #[inline] fn reduction_gt(self, other: Self) -> bool { self.value > other.value }
        }
    };
}
