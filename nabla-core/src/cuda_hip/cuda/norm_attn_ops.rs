...
macro_rules! dp {
    ($e:expr) => {
        &$e as *const CUdeviceptr as *mut c_void
    };
}
/// Cast u32 ref to kernel arg pointer.
macro_rules! up {
    ($e:expr) => {
        &$e as *const u32 as *mut c_void
    };
}
...
unsafe fn launch_with_typed_last<T: Scalar>(
    ...
    unsafe {
        ...
        if type_suffix::<T>() == "f32" {
            ...
            result::launch_kernel(func, grid, block, smem, stream, base_args)
                .or_panic("CUDA launch");
        } else {
...
pub(super) fn cuda_layer_norm<T: Scalar>(
    ...
    let func = expect_ok(
        get_kernel(
            ctx,
            kernel_name_buf(&mut nbuf, "layer_norm", type_suffix::<T>()),
        ),
        "CUDA kernel lookup",
    );
...
pub(super) fn cuda_rms_norm<T: Scalar>(
    ...
    let func = expect_ok(
        get_kernel(
            ctx,
            kernel_name_buf(&mut nbuf, "rms_norm", type_suffix::<T>()),
        ),
        "CUDA kernel lookup",
    );
    let out_buf = alloc_out::<T>(ctx, rows * cols);
...
pub(super) fn cuda_group_norm<T: Scalar>(
    ...
    let func = expect_ok(
        get_kernel(
            ctx,
            kernel_name_buf(&mut nbuf, "group_norm", type_suffix::<T>()),
        ),
        "CUDA kernel lookup",
    );
    let out_buf = alloc_out::<T>(ctx, rows * cols);
...
pub(super) fn cuda_batch_norm_train<T: Scalar>(
    ...
    let fwd_func = expect_ok(
        get_kernel(
            ctx,
            kernel_name_buf(&mut nbuf_fwd, "batch_norm_fwd", type_suffix::<T>()),
        ),
        "CUDA kernel lookup",
    );
    ...
    let (mean_ptr, var_ptr) = if training {
        ...
        let stats_func = expect_ok(
            get_kernel(
                ctx,
                kernel_name_buf(&mut nbuf_stats, "batch_norm_stats", type_suffix::<T>()),
            ),
            "CUDA kernel lookup",
        );
        ...
        unsafe {
            result::launch_kernel(
                stats_func,
                ((c_u32 + BLOCK_SIZE - 1) / BLOCK_SIZE, 1, 1),
                (BLOCK_SIZE, 1, 1),
                0,
                ctx.stream.cu_stream(),
                &mut [
        ...
        let mut upd_args = vec![
            dp!(running_mean.buf.ptr),
            dp!(running_var.buf.ptr),
            dp!(ms.buf.ptr),
            dp!(vs.buf.ptr),
        ];
...
pub(super) fn cuda_cross_entropy_fused<T: Scalar>(
    ...
    let func = expect_ok(
        get_kernel(
            ctx,
            kernel_name_buf(&mut nbuf, "cross_entropy", type_suffix::<T>()),
        ),
        "CUDA kernel lookup",
    );
    ...
    unsafe {
        result::launch_kernel(
            func,
            ((n_u32 + BLOCK_SIZE - 1) / BLOCK_SIZE, 1, 1),
            (BLOCK_SIZE, 1, 1),
            0,
            ctx.stream.cu_stream(),
            &mut [
...
pub(super) fn cuda_sdpa<T: Scalar>(
    ...
    let mut nbuf = [0u8; 64];
    let func = expect_ok(
        get_kernel(ctx, kernel_name_buf(&mut nbuf, "sdpa", type_suffix::<T>())),
        "CUDA kernel lookup",
    );
    let grid = batch_heads as u32 * seq_q.div_ceil(FA_BLOCK_M as usize) as u32;
    let smem_elem = if type_suffix::<T>() == "f64" {
        8usize
    } else {
        4
    };
...
pub(super) fn cuda_embedding<T: Scalar>(
    ...
    let func = expect_ok(
        get_kernel(
            ctx,
            kernel_name_buf(&mut nbuf, "embedding", type_suffix::<T>()),
        ),
        "CUDA kernel lookup",
    );
    let out_buf = alloc_out::<T>(ctx, total);
...
fn cuda_wht_impl<T: Scalar>(a: &CudaStorage<T>, inverse: bool) -> CudaStorage<T> {
    ...
    let mut nbuf = [0u8; 64];
    let func = expect_ok(
        get_kernel(ctx, kernel_name_buf(&mut nbuf, op, type_suffix::<T>())),
        "CUDA kernel lookup",
    );
    let out_buf = alloc_out::<T>(ctx, rows * cols);
    let (r, c) = (rows as u32, cols as u32);
    ...
    let smem = if type_suffix::<T>() == "bf16" {
        cols * 4
    } else {
        cols * std::mem::size_of::<T>()
    };
