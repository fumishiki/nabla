...
/// Dispatch cuBLAS strided-batched GEMM for conv ops (shared by conv1d/conv2d/conv3d).
/// # Safety
/// All buffer pointers must be valid GPU allocations with correct sizes.
#[allow(clippy::too_many_arguments)]
pub(super) unsafe fn conv_gemm_strided_batched<T: Scalar>(
    ctx: &CudaCtx,
    col_ptr: CUdeviceptr,
    weight_ptr: CUdeviceptr,
    out_ptr: CUdeviceptr,
    out_spatial: i32,
    c_out: i32,
    k_cols: i32,
    ...
    unsafe {
        if TypeId::of::<T>() == TypeId::of::<f32>() {
            ...
            panic!("{label}: unsupported scalar type (f32/f64/f16 only)");
        }
    }
}
...
pub(crate) fn cuda_sum_all<T: Scalar>(a: &CudaStorage<T>) -> T {
    ...
    read_scalar_from_device(&cuda_sum_all_1x1(a))
}

pub(crate) fn cuda_max_all<T: Scalar>(a: &CudaStorage<T>) -> T {
    read_scalar_from_device(&cuda_max_all_1x1(a))
}

pub(crate) fn cuda_min_all<T: Scalar>(a: &CudaStorage<T>) -> T {
    read_scalar_from_device(&cuda_min_all_1x1(a))
}

pub(crate) fn cuda_sum_all_1x1<T: Scalar>(a: &CudaStorage<T>) -> CudaStorage<T> {
    let ctx = get_ctx();
    if a.n() == 0 {
        return CudaStorage::new(1, 1, alloc_out::<T>(ctx, 1));
    }
    reduce_scalar_1x1(a, 0)
}

pub(crate) fn cuda_max_all_1x1<T: Scalar>(a: &CudaStorage<T>) -> CudaStorage<T> {
    reduce_scalar_1x1(a, 1)
}

pub(crate) fn cuda_min_all_1x1<T: Scalar>(a: &CudaStorage<T>) -> CudaStorage<T> {
    reduce_scalar_1x1(a, 2)
}

fn reduce_scalar_1x1<T: Scalar>(a: &CudaStorage<T>, func_base: usize) -> CudaStorage<T> {
    let ctx = get_ctx();
    let n = a.n();
    ...
    let n_u32 = n as u32;
    // SAFETY: kernel writes scalar result to out_buf on device (no pinned readback).
    unsafe {
...
pub(crate) fn cuda_prod_all<T: Scalar>(a: &CudaStorage<T>) -> T {
    ...
    let mut nbuf = [0u8; 64];
    let func_name = kernel_name_buf(&mut nbuf, "prod_partial", type_suffix::<T>());
    let func = expect_ok(get_kernel(ctx, func_name), "CUDA kernel lookup");
    let grid1 = ((n as u32) + BLOCK_SIZE - 1) / BLOCK_SIZE;
    let partial_buf = alloc_out::<T>(ctx, grid1 as usize);
    let n_u32 = n as u32;
    unsafe {
...
pub(super) fn cuda_softmax<T: Scalar>(a: &CudaStorage<T>) -> CudaStorage<T> {
    ...
    let n = rows * cols;
    let mut nbuf = [0u8; 64];
    let name = kernel_name_buf(&mut nbuf, "softmax", type_suffix::<T>());
...
pub(super) fn cuda_axis_reduce<T: Scalar>(a: &CudaStorage<T>, op: &str) -> CudaStorage<T> {
    ...
    let func = expect_ok(get_kernel(ctx, name), "CUDA kernel lookup");
    let out_buf = alloc_out::<T>(ctx, rows);
    let rows_u32 = rows as u32;
    let cols_u32 = cols as u32;
    unsafe {
        result::launch_kernel(
...
pub(super) fn cuda_im2col<T: Scalar>(
    ...
    let ctx = get_ctx();
    let mut nbuf = [0u8; 64];
    let name = kernel_name_buf(&mut nbuf, "im2col", type_suffix::<T>());
    let func = expect_ok(get_kernel(ctx, name), "CUDA kernel lookup");
    let col_buf = alloc_out::<T>(ctx, n * k_cols * out_spatial);
...
pub(super) fn cuda_conv_transpose2d<T: Scalar>(
    ...
    let ctx = get_ctx();
    let mut nbuf = [0u8; 64];
    let name = kernel_name_buf(&mut nbuf, "conv_transpose2d", type_suffix::<T>());
    let func = expect_ok(get_kernel(ctx, name), "CUDA kernel lookup");
...
pub(super) fn cuda_im1col<T: Scalar>(
    ...
    let ctx = get_ctx();
    let mut nbuf = [0u8; 64];
    let name = kernel_name_buf(&mut nbuf, "im1col", type_suffix::<T>());
    let func = expect_ok(get_kernel(ctx, name), "CUDA kernel lookup");
    let col_buf = alloc_out::<T>(ctx, n * k_cols * out_l);
...
pub(crate) fn cuda_conv1d<T: Scalar>(
    ...
    let ctx = get_ctx();
    let out_buf = alloc_out::<T>(ctx, n * c_out * out_l);
...
pub(super) fn cuda_adaptive_avg_pool2d<T: Scalar>(
    ...
    let total = nc * out_h * out_w;
    let mut nbuf = [0u8; 64];
    let name = kernel_name_buf(&mut nbuf, "adaptive_avg_pool2d", type_suffix::<T>());
    let func = expect_ok(get_kernel(ctx, name), "CUDA kernel lookup");
    let out_buf = alloc_out::<T>(ctx, total);
...
pub(super) fn cuda_max_pool2d_with_idx<T: Scalar>(
    ...
    let total = nc * out_h * out_w;
    let mut nbuf = [0u8; 64];
    let name = kernel_name_buf(&mut nbuf, "max_pool2d_with_idx", type_suffix::<T>());
    let func = expect_ok(get_kernel(ctx, name), "CUDA kernel lookup");
    let out_buf = alloc_out::<T>(ctx, total);
