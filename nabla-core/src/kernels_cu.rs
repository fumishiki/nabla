...
extern "C" __global__ void k_powf_f32(const float* in, float p, float* out, unsigned n) {
    unsigned i = THREAD_ID; if (i < n) out[i] = powf(in[i], p);
}
