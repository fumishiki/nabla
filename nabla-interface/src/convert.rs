...
pub fn map_tensor_name(name: &str) -> String {
    if name == "embedding.weight" { return "token_embd.weight".into(); }
    if name == "norm.weight" { return "output_norm.weight".into(); }
    if name == "output.weight" { return "output.weight".into(); }
    if let Some(dot_pos) = name.strip_prefix("layers.").and_then(|r| r.find('.').map(|p| 7 + p)) {
...
fn quantize_data(data: &[f32], qtype: GgufQuantType) -> Result<Vec<u8>> {
    if !qtype.is_quantizable() {
        return Err(Error::Quant(format!("{qtype:?} requires importance matrix or is not quantizable")));
    }
    quant::quantize(data, qtype)
}

fn resolve_qtype(name: &str, default: GgufQuantType, overrides: &[QuantOverride]) -> GgufQuantType {
    overrides.iter().find(|o| o.name == name).map_or(default, |o| o.qtype)
}
...
pub fn export_gguf(
    ...
    for &(name, tensor) in state_dict {
        ...
        let info = TensorInfo {
            name: gguf_name, dims: vec![ncols as u64, nrows as u64],
            qtype, data_size: quantized.len(),
        };
