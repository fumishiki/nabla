...
use nabla_interface::convert::{GgufArchConfig, QuantOverride, export_gguf, map_tensor_name};
use nabla_interface::quant::GgufQuantType;

#[test]
fn tensor_name_mapping_layers() {
    assert_eq!(
        map_tensor_name("layers.0.attention.wq.weight"),
        "blk.0.attn_q.weight"
    );
    assert_eq!(
        map_tensor_name("layers.5.ffn.w1.weight"),
        "blk.5.ffn_gate.weight"
    );
    assert_eq!(
        map_tensor_name("layers.2.ffn.w2.weight"),
        "blk.2.ffn_down.weight"
    );
    assert_eq!(
        map_tensor_name("layers.1.attention.wo.weight"),
...
fn export_gguf_two_layer_linear() {
    ...
    let w1 = Tensor::<f64, nabla_core::backend::DefaultBackend>::from_fn(4, 8, |r, c| {
        (r * 8 + c) as f64 * 0.01
    });
    let w2 = Tensor::<f64, nabla_core::backend::DefaultBackend>::from_fn(8, 4, |r, c| {
        (r * 4 + c) as f64 * 0.01
    });
...
fn export_with_quant_override() {
    ...
    let w = Tensor::<f64, nabla_core::backend::DefaultBackend>::from_fn(32, 32, |r, c| {
        (r * 32 + c) as f64 * 0.001
    });

    let state_dict: Vec<(&str, &Tensor<f64, nabla_core::backend::DefaultBackend>)> =
        vec![("layers.0.attention.wq.weight", &w)];
    ...
    export_gguf(&state_dict, &tmp, GgufQuantType::Q8_0, &config, &overrides)
