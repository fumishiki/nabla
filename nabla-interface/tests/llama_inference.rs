...
fn generate_text() {
    ...
    let config = InferenceConfig { n_ctx: 512, n_batch: 128, n_threads: 4, n_gpu_layers: -1 };
    let mut engine = InferenceEngine::new(&gguf_path, config).expect("engine creation failed");

    let output = engine.generate("Hello", 32, &SamplingConfig::default()).expect("generate failed");
