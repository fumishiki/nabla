...
enum ContractionKind {
    ...
    Trace,
    /// Outer product: c[i,j] = a[i] * b[j] (no contraction, vector x vector)
    Outer,
...
fn validate(input: &EinsumInput) -> Result<()> {
    ...
    // Contraction indices must appear in at least 2 terms (or be trace pattern).
    let free_names = name_set(&input.output_indices);
    let idx_term_count = contraction_counts(input, &free_names);
    for (name, (count, ident)) in &idx_term_count {
        if *count != 1 {
            continue;
        }
        // Allow trace pattern: same index twice within a single term.
        let is_trace = is_trace_index(input, name);
        if !is_trace {
            return Err(Error::new_spanned(
                ident,
                format!(
...
fn contraction_counts<'a>(
    ...
    for term in &input.rhs_terms {
        for idx in &term.indices {
            ...
            if let Some((count, _)) = counts.get_mut(&name) {
                *count += 1;
            } else {
                counts.insert(name, (1, idx));
            }
        }
    }
    counts
...
fn classify(input: &EinsumInput) -> ContractionKind {
    ...
    let mut contraction_seen = HashSet::new();
    for term in rhs {
        for idx in &term.indices {
            let s = idx.to_string();
            if !free_names.contains(&s) && contraction_seen.insert(s.clone()) {
                contraction.push(s);
            }
        }
    }
    ...
    match free_indices.len() {
        ...
        _ => {
            ...
            for fi in free_indices {
                shape_exprs.push(dim_lookup(fi)?);
            }
...
fn codegen_hadamard(input: &EinsumInput) -> Result<TokenStream2> {
    ...
    let a_idx = name_list(&input.rhs_terms[0].indices);
    let b_idx = name_list(&input.rhs_terms[1].indices);
...
fn codegen_batch_gemm(input: &EinsumInput, batch_count: usize) -> Result<TokenStream2> {
    ...
    shape_exprs.push(quote! { #a_bind.dim(#rd) });
    shape_exprs.push(quote! { #b_bind.dim(#cd) });
    ...
    let inner = quote! {
        ...
        __out.set_slice_2d(#batch_idx, &__c_slice);
    };
    ...
    Ok(quote! {
        {
            #a_stmt
            #b_stmt
            let mut __out = nabla::tensor::NdTensor::zeros(&[#(#shape_exprs),*]);
            #loops
            __out
        }
    })
...
fn codegen_fallback(input: &EinsumInput) -> Result<TokenStream2> {
    ...
    let rhs = &input.rhs_terms;
    let free_names = name_list(&input.output_indices);
    ...
    for term in rhs {
        for idx in &term.indices {
            let s = idx.to_string();
            if !free_names.contains(&s) && seen_c.insert(s) {
    ...
    let accesses: Vec<TokenStream2> = rhs
        .iter()
        .map(|t| {
            let binding = Ident::new(&format!("__{}", t.name), t.name.span());
    ...
    for _ in 0..terms.len() - 1 {
        ...
        for i in 0..alive.len() {
            for j in (i + 1)..alive.len() {
                ...
                let (left, right) = (&slot_indices[li], &slot_indices[ri]);
...
fn codegen_sequential_contraction(input: &EinsumInput) -> Result<TokenStream2> {
    ...
    // Path annotation for generated code readability.
    ...
    // Slot map: original terms + generated intermediates.
    ...
    for (step_idx, &(li, ri)) in contraction_order.iter().enumerate() {
        ...
        let (acc, rhs_t) = match (left, right) {
            (Some(l), Some(r)) => (l, r),
            _ => {
        ...
        // Remaining indices in unconsumed slots.
        ...
        // Step output indices = union minus contracted, order preserved.
        let mut step_out: Vec<Ident> = Vec::new();
        let mut step_seen: HashSet<String> = HashSet::new();
        for idx in acc.indices.iter().chain(rhs_t.indices.iter()) {
            let s = idx.to_string();
            if !contracted.contains(&s) && step_seen.insert(s) {
                step_out.push(idx.clone());
            }
        }

        // Deduplicated contraction idents.
        let mut contr_dedup: Vec<&Ident> = Vec::new();
        let mut seen_c: HashSet<String> = HashSet::new();
        for idx in acc.indices.iter().chain(rhs_t.indices.iter()) {
            if contracted.contains(&idx.to_string()) && seen_c.insert(idx.to_string()) {
                contr_dedup.push(idx);
            }
        }

        // Element access + product.
        ...
        let product = quote! { nabla::scalar::math_utils::mul(&#l_access, &#r_access) };
        ...
        // Build dimension lookup descriptors for this binary step.
        let l_bind = &acc.binding;
        let r_bind = &rhs_t.binding;
        let l_indices = &acc.indices;
        let r_indices = &rhs_t.indices;
        let l_orig = acc.is_original;
        let r_orig = rhs_t.is_original;
        let pair: Vec<(&Ident, &[Ident], bool)> = vec![
            (l_bind, l_indices.as_slice(), l_orig),
        ...
        if is_final {
            ...
            // Re-emit with correct name.
            let last = intermediate_stmts.len() - 1;
            intermediate_stmts[last] = quote! {
                let mut __acc = nabla::scalar::math_utils::zero::<_>();
                #loops
                let #tmp_name = __acc;
            };
            ...
            let mut shape_exprs = Vec::new();
            for fi in &step_out {
                shape_exprs.push(find_dim_expr(fi, &pair, None)?);
            }
