...
fn detect_block_sizes(t: &[f64], n: usize) -> Vec<usize> {
    ...
    let mut i = 0;
    while i < n {
        if i + 1 < n && t[(i + 1) * n + i].abs() > f64::EPSILON * 4.0 * (t[i * n + i].abs() + t[(i + 1) * n + i + 1].abs()).max(f64::EPSILON) {
            blocks.push(2);
            i += 2;
        } else {
            blocks.push(1);
...
fn solve_4x4(mat: &[f64; 16], rhs: &[f64; 4]) -> Result<[f64; 4]> {
    ...
    for col in 0..4 {
        let pivot_row = (col..4)
            .max_by(|&r1, &r2| a[r1 * 4 + col].abs().partial_cmp(&a[r2 * 4 + col].abs()).unwrap_or(std::cmp::Ordering::Equal))
            .unwrap_or(col);
        if pivot_row != col {
            for k in 0..4 {
...
fn solve_triangular_sylvester(
    ...
    for &bj in &tb_blocks {
        ...
        for jc in jj..(jj + bj) {
            for k in 0..jj {
                let tb_kj = tb[k * m + jc];
                if tb_kj.abs() < f64::EPSILON { continue; }
                for i in 0..n {
        ...
        for &bi in ta_blocks.iter().rev() {
            ...
            if bi == 1 && bj == 1 {
                ...
                let (y0, y1) = solve_2x2(
                    ta[ii * n + ii] + shift, ta[ii * n + ii + 1],
                    ta[(ii + 1) * n + ii], ta[(ii + 1) * n + ii + 1] + shift,
                    f[ii * m + jj], f[(ii + 1) * m + jj],
                )?;
                f[ii * m + jj] = y0;
                f[(ii + 1) * m + jj] = y1;
            } else if bi == 1 && bj == 2 {
                ...
                let d = ta[ii * n + ii];
                let (y0, y1) = solve_2x2(
                    d + tb[jj * m + jj], tb[(jj + 1) * m + jj],
                    tb[jj * m + jj + 1], d + tb[(jj + 1) * m + jj + 1],
                    f[ii * m + jj], f[ii * m + jj + 1],
                )?;
                f[ii * m + jj] = y0;
                f[ii * m + jj + 1] = y1;
            } else {
                ...
                let rhs_vec = [f[i0 * m + j0], f[i1 * m + j0], f[i0 * m + j1], f[i1 * m + j1]];
    ...
    for &bj in &tb_blocks {
        ...
        for &bi in ta_blocks.iter().rev() {
            ...
            if bi == 1 && bj == 1 {
                ...
                let shift = tb[jj * m + jj];
                let (y0, y1) = solve_2x2(
                    1.0 - shift * ta[ii * n + ii], -shift * ta[ii * n + ii + 1],
                    -shift * ta[(ii + 1) * n + ii], 1.0 - shift * ta[(ii + 1) * n + ii + 1],
                    f[ii * m + jj], f[(ii + 1) * m + jj],
                )?;
                f[ii * m + jj] = y0;
                f[(ii + 1) * m + jj] = y1;
                ...
                let (y0, y1) = solve_2x2(
                    1.0 - b00 * a_ii, -b10 * a_ii,
                    -b01 * a_ii, 1.0 - b11 * a_ii,
                    f[ii * m + jj], f[ii * m + jj + 1],
                )?;
                f[ii * m + jj] = y0;
                f[ii * m + jj + 1] = y1;
                ...
                let mat = [
                    1.0 - b00 * a00, -b00 * a01,      -b10 * a00,      -b10 * a01,
                    -b00 * a10,       1.0 - b00 * a11, -b10 * a10,      -b10 * a11,
                    -b01 * a00,       -b01 * a01,       1.0 - b11 * a00, -b11 * a01,
                    -b01 * a10,       -b01 * a11,       -b11 * a10,       1.0 - b11 * a11,
                ];
                let rhs_vec = [f[i0 * m + j0], f[i1 * m + j0], f[i0 * m + j1], f[i1 * m + j1]];
                let x = solve_4x4(&mat, &rhs_vec)?;
                f[i0 * m + j0] = x[0];
