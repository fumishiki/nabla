...
impl GgufQuantType {
    ...
    pub const fn block_size(self) -> usize {
        ...
        match self {
            ...
            | IQ2_XXS | IQ2_XS | IQ2_S | IQ3_XXS | IQ3_S | IQ4_XS | IQ1_S | IQ1_M | TQ1_0
            | TQ2_0 => QK_K,
        }
    }
...
impl GgufValue {
    fn type_id(&self) -> u32 {
        match self {
            ...
            Self::Bool(_) => 7,
            Self::Str(_) => 8,
            Self::ArrayU32(_) | Self::ArrayI32(_) | Self::ArrayF32(_) | Self::ArrayStr(_) => 9,
            Self::U64(_) => 10,
            Self::I64(_) => 11,
            Self::F64(_) => 12,
        }
...
pub fn quantize_q2_k(data: &[f32], out: &mut Vec<u8>) {
    ...
    for block in data.chunks_exact(QK_K) {
        ...
        for (sb, sub) in block.chunks_exact(16).enumerate() {
            ...
            let s = if range != 0.0 {
                ((d_sb * 15.0 / range).clamp(0.0, 15.0) as u8) & 0x0F
            } else {
                0
            };
            let m_q = if range != 0.0 {
...
pub fn quantize_q4_k(data: &[f32], out: &mut Vec<u8>) {
    ...
    for block in data.chunks_exact(QK_K) {
        ...
        for (sb, sub) in block.chunks_exact(32).enumerate() {
            ...
            let sc = if d != 0.0 {
                (sb_max / d).clamp(0.0, 63.0) as u8
            } else {
                0
            };
            let mn = if dmin_val != 0.0 {
                (sb_min.abs() / dmin_val).clamp(0.0, 63.0) as u8
            } else {
                0
            };
        ...
        _ => {
            ...
            match qtype {
                GgufQuantType::Q3_K_S | GgufQuantType::Q3_K_M | GgufQuantType::Q3_K_L => {
                    quantize_q2_k(data, &mut out);
                } // conservative fallback
                GgufQuantType::Q5_K_S | GgufQuantType::Q5_K_M => quantize_q6_k(data, &mut out),
...
pub fn dequantize(data: &[u8], n: usize, qtype: GgufQuantType) -> Vec<f32> {
    ...
    match qtype {
        ...
        _ => match qtype {
            ...
            GgufQuantType::Q5_K_S | GgufQuantType::Q5_K_M => dequantize_q6_k(data, n, &mut out),
...
impl ImportanceMatrix {
    ...
    pub fn accumulate(&mut self, name: &str, activations: &[f32]) {
        let entry = self
            .scores
            .entry(name.to_owned())
            .or_insert_with(|| vec![0.0; activations.len()]);
        for (s, &a) in entry.iter_mut().zip(activations) {
            *s += a * a;
        }
    }
...
pub fn write_gguf<W: Write + Seek>(
    ...
    for t in tensors {
        ...
        if rem != 0 {
            w.write_all(&vec![0u8; (DEFAULT_ALIGNMENT - rem) as usize])?;
        }
    }
    Ok(())
}
...
pub fn export_gguf<W: Write + Seek>(
    ...
    let mut metadata: Vec<(&str, GgufValue)> = vec![
        (
            ...
            GgufValue::Str(config.model_arch.clone()),
        ),
        (
            "general.file_type",
            GgufValue::U32(config.base_quant as u32),
        ),
    ];
    let extra_refs: Vec<(&str, GgufValue)> = config
        .extra_metadata
        .iter()
        .map(|(k, v)| (k.as_str(), v.clone()))
        .collect();
    metadata.extend_from_slice(&extra_refs);
    let tensors: Vec<GgufTensor> = weights
        .iter()
        .map(|&(name, dims, data)| {
            let qtype = config.mixing.map_or(config.base_quant, |preset| {
                mixing_quant_type(name, config.base_quant, preset)
            });
