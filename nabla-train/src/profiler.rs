...
impl HardwareSpec {
    ...
    pub fn new(peak_tflops: f64, peak_bandwidth_tb_s: f64) -> Self {
        Self { peak_tflops, peak_bandwidth_tb_s }
    }

    /// GH200 preset: 495 TFLOPS TF32, 4.0 TB/s HBM3e.
    #[must_use]
    pub fn gh200() -> Self { Self::new(495.0, 4.0) }

    /// A100 80GB preset: 312 TFLOPS TF32, 2.0 TB/s HBM2e.
    #[must_use]
    pub fn a100_80gb() -> Self { Self::new(312.0, 2.0) }

    /// H100 SXM preset: 989 TFLOPS TF32, 3.35 TB/s HBM3.
    #[must_use]
    pub fn h100_sxm() -> Self { Self::new(989.0, 3.35) }
}

/// Whether a kernel is compute-bound or memory-bound (P6-PROF-05).
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum BottleneckKind { Compute, Memory }

impl BottleneckKind {
    fn as_str(self) -> &'static str {
        match self { Self::Compute => "compute", Self::Memory => "memory" }
    }
}
...
pub fn roofline(hw: &HardwareSpec, flops: f64, bytes_transferred: f64) -> RooflineResult {
    let ai = if bytes_transferred > 0.0 { flops / bytes_transferred } else { f64::INFINITY };
    let memory_roof = ai * hw.peak_bandwidth_tb_s * 1e12 / 1e12;
    let attainable = hw.peak_tflops.min(memory_roof);
    let bottleneck = if memory_roof < hw.peak_tflops { BottleneckKind::Memory } else { BottleneckKind::Compute };
    RooflineResult { arithmetic_intensity: ai, attainable_tflops: attainable, bottleneck }
}
...
impl ThroughputStats {
    ...
    pub fn compute(total_tokens: u64, batch_size: u64, elapsed_ms: f64) -> Self {
        ...
        let tok_s = if elapsed_s > 0.0 { total_tokens as f64 / elapsed_s } else { 0.0 };
        let ms_tok = if total_tokens > 0 { elapsed_ms / total_tokens as f64 } else { 0.0 };
        let batch_tp = if elapsed_s > 0.0 { batch_size as f64 / elapsed_s } else { 0.0 };
        Self { tokens_per_sec: tok_s, ms_per_token: ms_tok, batch_throughput: batch_tp }
    }
}

/// Matmul FLOPS: 2*M*K*N (P6-PROF-04).
#[must_use]
pub fn matmul_flops(m: u64, k: u64, n: u64) -> f64 { 2.0 * m as f64 * k as f64 * n as f64 }
...
impl Profiler {
    ...
    pub fn new() -> Self {
        Self {
            kernels: Vec::new(), layers: Vec::new(), vram: VramStats::default(),
            hw: None, throughput_timer: None, throughput_tokens: 0, throughput_batch: 0,
        }
    }

    #[must_use]
    pub fn with_hardware(mut self, hw: HardwareSpec) -> Self { self.hw = Some(hw); self }

    /// Record a kernel timing (P6-PROF-01).
    pub fn record_kernel(&mut self, name: impl Into<String>, elapsed_ms: f64, flops: Option<f64>, bytes: Option<f64>) {
        self.kernels.push(KernelRecord { name: name.into(), elapsed_ms, flops, bytes });
    }

    /// Record per-layer stats (P6-PROF-02).
    pub fn record_layer(&mut self, name: impl Into<String>, forward_ms: f64, backward_ms: f64, vram_bytes: u64, tflops: Option<f64>) {
        self.layers.push(LayerStats { name: name.into(), forward_ms, backward_ms, vram_bytes, tflops });
    }

    /// Update VRAM tracking (P6-PROF-06).
    pub fn update_vram(&mut self, current: u64, peak: u64) {
        self.vram.current_bytes = current;
        if peak > self.vram.peak_bytes { self.vram.peak_bytes = peak; }
    }
    ...
    pub fn add_tokens(&mut self, n: u64) { self.throughput_tokens += n; }

    /// Finalize and return throughput stats (P6-PROF-03).
    #[must_use]
    pub fn throughput(&self) -> Option<ThroughputStats> {
        self.throughput_timer.map(|t| {
            ThroughputStats::compute(self.throughput_tokens, self.throughput_batch, t.elapsed().as_secs_f64() * 1000.0)
        })
    }
    ...
    pub fn kernel_tflops(&self) -> Vec<(&str, Option<f64>)> {
        self.kernels.iter().map(|k| (k.name.as_str(), k.tflops())).collect()
    }
    ...
    pub fn kernel_breakdown(&self) -> Vec<(String, f64, usize)> {
        ...
        let mut out: Vec<_> = map.into_iter().map(|(name, (ms, count))| (name, ms, count)).collect();
        out.sort_by(|a, b| b.1.total_cmp(&a.1));
        out
    }

    pub fn kernels(&self) -> &[KernelRecord] { &self.kernels }
    pub fn layers(&self) -> &[LayerStats] { &self.layers }
    pub fn vram(&self) -> &VramStats { &self.vram }
    ...
    pub fn to_json(&self) -> String {
        ...
        for (i, l) in self.layers.iter().enumerate() {
            ...
            let _ = write!(s,
                ...
                json_escape(&l.name), l.forward_ms, l.backward_ms, l.vram_bytes,
                l.tflops.map_or("null".to_string(), |v| format!("{v:.4}")),
            );
            if i + 1 < self.layers.len() { s.push(','); }
            s.push('\n');
        }
        s.push_str("  ],\n");
        // Roofline per kernel
        ...
        for (i, entry) in roofline_entries.iter().enumerate() {
            s.push_str(entry);
            if i + 1 < roofline_entries.len() { s.push(','); }
            s.push('\n');
        }
        s.push_str("  ],\n");
        // VRAM
        {
            ...
            for (i, (name, bytes)) in self.vram.per_layer.iter().enumerate() {
                let _ = write!(s, "{{\"name\":\"{}\",\"bytes\":{bytes}}}", json_escape(name));
                if i + 1 < self.vram.per_layer.len() { s.push(','); }
            }
        }
        s.push_str("]},\n");
        // Throughput
