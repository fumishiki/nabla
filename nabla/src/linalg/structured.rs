// Advanced decompositions and structured matrix constructors:
// hessenberg, polar, toeplitz, circulant, vandermonde, balance,
// frechet_deriv, continuous_riccati.
...
// ---------------------------------------------------------------------------
// hessenberg — Public Hessenberg decomposition  A = Q·H·Q^T
// ---------------------------------------------------------------------------

/// Reduce a square matrix to upper Hessenberg form: `A = Q·H·Q^T`.
///
/// Returns `(Q, H)` where `H` is upper Hessenberg and `Q` is orthogonal.
/// The decomposition uses successive Householder reflections and accumulates
/// the orthogonal factor `Q` explicitly.  Only `f64` / `Cpu` backend.
///
/// # Errors
///
...
// ---------------------------------------------------------------------------
// polar — Polar decomposition  A = U·H  via SVD
// ---------------------------------------------------------------------------

/// Polar decomposition `A = U·H` where `U` is orthogonal and `H` is
/// symmetric positive semi-definite.
///
/// Computed via SVD: `A = U_svd·Σ·V^T`, then
/// `U = U_svd·V^T` and `H = V·Σ·V^T`.  Only `f64` / `Cpu` backend.
///
/// # Errors
///
/// Returns `Err` if the SVD fails to converge.
pub fn polar(
    a: &Tensor<f64, Cpu>,
) -> Result<(Tensor<f64, Cpu>, Tensor<f64, Cpu>)> {
    let (m, n) = a.shape();
...
// ---------------------------------------------------------------------------
// toeplitz — Build a Toeplitz matrix
// ---------------------------------------------------------------------------

/// Build an `m × n` Toeplitz matrix from first column `col` (length `m`) and
/// first row `row` (length `n`).
///
/// `T[i, j] = col[i - j]` when `i >= j`, and `T[i, j] = row[j - i]` when
/// `j > i`.  `col[0]` must equal `row[0]`.
///
/// # Panics
///
/// Does **not** panic; returns a zero matrix if slices are empty.
#[must_use]
...
// ---------------------------------------------------------------------------
// circulant — Build a circulant matrix
// ---------------------------------------------------------------------------

/// Build an `n × n` circulant matrix from the first-column vector `c`
/// (length `n`).
///
/// `C[i, j] = c[(i - j).rem_euclid(n)]`.
///
/// # Panics
///
/// Does **not** panic; returns an empty matrix when `c` is empty.
#[must_use]
...
// ---------------------------------------------------------------------------
// vandermonde — Build a Vandermonde matrix
// ---------------------------------------------------------------------------

/// Build an `m × m` Vandermonde matrix from node vector `nodes` (length `m`).
///
/// `V[i, j] = nodes[i]^j`.
///
/// # Panics
///
/// Does **not** panic; returns an empty matrix when `nodes` is empty.
#[must_use]
...
/// Balance a square matrix by diagonal similarity `D^{-1}·A·D` so that row
/// and column norms are approximately equal.
///
/// Uses powers of 2 for the scaling factors (exact in floating-point
/// arithmetic).  Returns `(B, scale)` where `B = D^{-1}·A·D` and `scale[i]`
/// is the diagonal entry of `D` for row/column `i`.
///
...
// ---------------------------------------------------------------------------
// frechet_deriv — Frechet derivative via block-triangular embedding
// ---------------------------------------------------------------------------

/// Compute the Fréchet derivative `L_f(A)[E]` of a matrix function `f` at `A`
/// in the direction `E`.
///
/// Uses the block-triangular identity:
/// ```text
/// f([[A, E], [0, A]]) = [[f(A), L_f(A)[E]], [0, f(A)]]
/// ```
/// so the upper-right `n × n` block of `f` applied to the `2n × 2n` block
/// matrix gives the Fréchet derivative.
///
/// Only `f64` / `Cpu` backend.
///
/// # Errors
///
/// Returns `Err` if `A` and `E` are not the same square shape, or `f` returns
/// an error.
...
// ---------------------------------------------------------------------------
// continuous_riccati — Continuous Algebraic Riccati Equation (CARE)
// ---------------------------------------------------------------------------

/// Solve the Continuous Algebraic Riccati Equation (CARE):
/// `A^T·X + X·A - X·B·R^{-1}·B^T·X + Q = 0`.
///
/// Algorithm (Hamiltonian Schur):
/// 1. Form the `2n × 2n` Hamiltonian `H = [[A, -B·R^{-1}·B^T], [-Q, -A^T]]`.
/// 2. Compute the real Schur decomposition `H = U·T·U^T`.
/// 3. Reorder columns of `U` to isolate the stable invariant subspace
///    (eigenvalues with negative real part).
/// 4. Partition the reordered `U` as `[[U11], [U21]]`; then `X = U21·U11^{-1}`.
///
/// Only `f64` / `Cpu` backend.
///
/// # Errors
///
/// Returns `Err` if dimension checks fail, the Schur decomposition does not
/// converge, `R` is singular, or the stable subspace is not of dimension `n`
/// (Hamiltonian has eigenvalues on the imaginary axis).
...
/// Alias for [`continuous_riccati`] — solves the Continuous Algebraic Riccati
/// Equation (CARE): `A^T·X + X·A - X·B·R^{-1}·B^T·X + Q = 0`.
///
/// # Errors
///
/// Propagates all errors from [`continuous_riccati`].
#[inline]
pub fn care(
    a: &Tensor<f64, Cpu>,
    b: &Tensor<f64, Cpu>,
