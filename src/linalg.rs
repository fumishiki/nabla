// linalg.rs — Self-contained dense linear algebra factorizations for Tensor<T, Cpu>.
//
...
macro_rules! impl_solve_in_place {
    ($fn_name:ident, $solve_method:ident) => {
        ...
        pub fn $fn_name(&self, rhs: &mut Self) -> Result<()> {
...
impl Qr<f64> {
    ...
    fn factorize(a: &Tensor<f64, Cpu>) -> Self {
        ...
        for j in 0..k {
            ...
            // Store Householder vector (below diagonal) in column j
            for (i, vi) in v.iter().enumerate().skip(1).take(col_len - 1) {
                buf_set(&mut buf, n, i + j, j, *vi);
            }
        }
...
impl Svd<f64> {
    ...
    fn bidiag_qr_svd(
        ...
        while p > 0 {
            ...
            // Wilkinson shift
            let d_p = d[p - 1];
            let d_pm1 = if p > 1 { d[p - 2] } else { d[0] };
            let e_pm1 = if p > 1 { e[p - 2] } else { 0.0 };
            let t =
                (d_pm1 * d_pm1 + e_pm1 * e_pm1 - d_p * d_p) / (2.0 * d_p * e_pm1 + f64::EPSILON);
            let mu = d_p * d_p - d_p * e_pm1 / (t + t.signum() * t.hypot(1.0) + f64::EPSILON);

            let mut f = d[q] * d[q] - mu;
            let mut g = d[q] * if q < e.len() { e[q] } else { 0.0 };

            for i in q..p.saturating_sub(1) {
                let r = f.hypot(g);
                let c = if r == 0.0 { 1.0 } else { f / r };
                let s = if r == 0.0 { 0.0 } else { g / r };
                if i > q {
                    e[i - 1] = r;
                }
                f = c * d[i] + s * if i < e.len() { e[i] } else { 0.0 };
                let e_i = if i < e.len() { e[i] } else { 0.0 };
                g = s * d[i + 1];
                let d_next = c * d[i + 1];
                // Apply Givens to rows of Vt
                for col in 0..n {
                    let v0 = vt[i * n + col];
                    let v1 = vt[(i + 1) * n + col];
                    vt[i * n + col] = c * v0 + s * v1;
                ...
                let r2 = f.hypot(g);
                let c2 = if r2 == 0.0 { 1.0 } else { f / r2 };
                let s2 = if r2 == 0.0 { 0.0 } else { g / r2 };
                d[i] = r2;
                f = c2 * e_i + s2 * d_next;
                d[i + 1] = -s2 * e_i + c2 * d_next;
                if i + 1 < e.len() {
                    g = s2 * e[i + 1];
                    e[i + 1] *= c2;
                }
                // Apply Givens to columns of U
                for row in 0..m {
                    let u0 = u[row * m + i];
                    let u1 = u[row * m + i + 1];
                    u[row * m + i] = c * u0 + s * u1;
                    u[row * m + i + 1] = -s * u0 + c * u1;
                }
                if i < e.len() {
                    e[i] = f;
                }
...
// ===========================================================================
// Tensor<f64, Cpu> impl — all factorization and solve methods
// ===========================================================================
...
impl Tensor<f64, Cpu> {
    ...
    pub fn partial_piv_lu(&self) -> Result<PartialPivLu<f64>> {
    ...
    /// # Errors
    /// Returns `Err` when the matrix is singular or not square.
    pub fn full_piv_lu(&self) -> Result<FullPivLu<f64>> {
        FullPivLu::factorize(self)
    }

    /// QR decomposition (`A = QR`).
    #[must_use]
    pub fn qr(&self) -> Qr<f64> {
        Qr::factorize(self)
    }

    /// Column-pivoted QR decomposition (`A·P^T = QR`).
    #[must_use]
    pub fn col_piv_qr(&self) -> ColPivQr<f64> {
    ...
    pub fn llt(&self, side: Side) -> Result<Llt<f64>> {
    ...
    pub fn ldlt(&self, side: Side) -> Result<Ldlt<f64>> {
        Ldlt::factorize(self, side)
    }

    /// Bunch-Kaufman `LBL^T` decomposition.
    #[must_use]
    pub fn lblt(&self, side: Side) -> Lblt<f64> {
    ...
    pub fn svd(&self) -> Result<Svd<f64>> {
        Svd::factorize(self)
    }
    ...
    /// # Errors
    /// Returns `Err` when SVD fails to converge.
    pub fn thin_svd(&self) -> Result<Svd<f64>> {
        Svd::factorize(self)
    }
    ...
    pub fn self_adjoint_eigen(&self, side: Side) -> Result<SelfAdjointEigen<f64>> {
    ...
    /// # Errors
    /// Returns `Err` if the matrix is not square or eigensolver fails.
    pub fn self_adjoint_eigenvalues(&self, side: Side) -> Result<Vec<f64>> {
    ...
    pub fn partial_piv_lu_reconstruct(&self) -> Result<Self> {
    ...
    /// # Errors
    /// Returns `Err` when factorization fails.
    pub fn partial_piv_lu_inverse(&self) -> Result<Self> {
