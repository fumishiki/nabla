//! GPU backend integration tests (wgpu).
#![cfg(feature = "wgpu")]
...
fn tensors_close_f32(gpu: &Tensor<f32, Wgpu>, cpu: &Tensor<f32, Cpu>) {
...
fn zeros_f32_shape_and_elements() {
    let t = Tensor::<f32, Wgpu>::zeros(3, 4);
...
fn from_fn_f64_values() {
    let t = Tensor::<f64, Wgpu>::from_fn(2, 3, |i, j| (i * 3 + j) as f64);
...
fn get_after_from_fn() {
    let t = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i as f32) * 10.0 + j as f32);
...
#[test]
fn set_then_get() {
    let mut t = Tensor::<f32, Wgpu>::zeros(3, 3);
    t.set(1, 2, 42.0f32);
...
fn add_f32_matches_cpu() {
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i * 4 + j) as f32);
    let b_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i + j) as f32);
...
// f64 GPU path requires FLOAT64 capability (available on CUDA, not wgpu/Metal).
// f64 on wgpu uses CPU fallback — covered by from_fn_f64_values test.

#[test]
fn sub_f32_matches_cpu() {
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i * 4 + j + 1) as f32);
    let b_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |_i, j| (j + 1) as f32);
...
fn neg_f32_matches_cpu() {
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(3, 4, |i, j| (i as f32) - (j as f32) * 0.5);
...
fn transpose_f32_shape_and_values() {
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(3, 5, |i, j| (i * 5 + j) as f32);
...
fn matmul_f32_matches_cpu() {
    // 2x3 * 3x2 = 2x2
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(2, 3, |i, j| (i * 3 + j + 1) as f32);
    let b_gpu = Tensor::<f32, Wgpu>::from_fn(3, 2, |i, j| (i * 2 + j + 1) as f32);
    let a_cpu = Tensor::<f32, Cpu>::from_fn(2, 3, |i, j| (i * 3 + j + 1) as f32);
    let b_cpu = Tensor::<f32, Cpu>::from_fn(3, 2, |i, j| (i * 2 + j + 1) as f32);
    tensors_close_f32(&(&a_gpu * &b_gpu), &(&a_cpu * &b_cpu));
}

// f64 matmul on wgpu uses CPU fallback (FLOAT64 capability requires CUDA).
...
fn chained_add_matmul_f32() {
    ...
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(3, 3, |i, j| (i + j) as f32);
    let b_gpu = Tensor::<f32, Wgpu>::from_fn(3, 3, |i, j| (i * j) as f32);
    let d_gpu = Tensor::<f32, Wgpu>::from_fn(3, 3, |i, j| if i == j { 2.0 } else { 1.0 });
...
fn exp_f32_matches_cpu() {
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i * 4 + j) as f32 * 0.1);
...
fn ln_f32_matches_cpu() {
    ...
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i * 4 + j + 1) as f32 * 0.5);
...
fn log1p_f32_matches_cpu() {
    // values > -1
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i * 4 + j) as f32 * 0.25);
...
fn sin_f32_matches_cpu() {
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i * 4 + j) as f32 * 0.2);
...
fn tanh_f32_matches_cpu() {
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i as f32) - (j as f32) * 0.5);
...
fn abs_f32_matches_cpu() {
    ...
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i as f32) - (j as f32) * 1.5);
...
fn erf_f32_matches_cpu() {
    ...
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i as f32) * 0.5 - (j as f32) * 0.3);
...
fn ceil_f32_matches_cpu() {
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i * 4 + j) as f32 * 0.7 - 5.0);
...
fn powf_f32_matches_cpu() {
    ...
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(3, 3, |i, j| 1.0f32 + (i * 3 + j) as f32 * 0.1);
...
fn mul_elem_f32_matches_cpu() {
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i * 4 + j) as f32 * 0.5);
    let b_gpu = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i + j + 1) as f32);
...
fn chained_exp_mul_f32() {
    ...
    let a_gpu = Tensor::<f32, Wgpu>::from_fn(3, 3, |i, j| (i + j) as f32 * 0.5);
    let b_gpu = Tensor::<f32, Wgpu>::from_fn(3, 3, |i, j| (i * j + 1) as f32);
...
fn clone_produces_equal_tensor() {
    let a = Tensor::<f32, Wgpu>::from_fn(4, 4, |i, j| (i * 4 + j) as f32);
    let b = a.clone();
...
fn clone_is_independent() {
    let a = Tensor::<f32, Wgpu>::from_fn(3, 3, |i, j| (i * 3 + j) as f32);
    let mut b = a.clone();
    b.set(0, 0, 99.0);
